...
class Clientes(tk.Frame):
    ...
    def widgets(self) -> None:
        ...
        frame1 = tk.Frame(frame_principal, bg=COLOR_FONDO, padx=10, pady=10)
        frame1.grid(row=0, column=0, sticky="nsew")

        # Frame derecho (70% expandible)
        frame2 = tk.Frame(frame_principal, bg=COLOR_FONDO, padx=10, pady=10)
        ...
        # Campos de entrada
        frame_form = tk.Frame(frame1, bg=COLOR_FONDO)
        ...
        for text, key in campos:
            frame_campo = tk.Frame(frame_form, bg=COLOR_FONDO)
        ...
        def cargar_img(nombre: str) -> ImageTk.PhotoImage | None:
            try:
                ...
                registrador.error("No se pudo cargar la imagen %s", nombre)
                return None
        ...
        # Botón Limpiar
        self.btn_limpiar = crear_boton(
            lf_opciones,
            "  Limpiar",
            estilo=BOTON_NEUTRO,
            image=img_clear,
            compound="left",
            comando=self.limpiar_campos,
            padx=10,
            anchor="w",
        )
        ...
        frame_tabla = tk.Frame(frame2, bg=COLOR_FONDO)
        ...
        self.frame_paginacion = tk.Frame(frame2, bg=COLOR_FONDO)
    ...
    def _actualizar_totales(self) -> None:
        ...
        self.total_paginas = max(
            1,
            (total_clientes + self.clientes_por_pagina - 1) // self.clientes_por_pagina,
        )
    ...
    def _cargar_pagina(self) -> None:
        ...
        clientes = ClientesServicio.obtener_clientes_paginado(
            offset=offset, limit=self.clientes_por_pagina, filtro=self.filtro_actual
        )
        self._mostrar_clientes_en_treeview(clientes)
        self._actualizar_etiqueta_paginacion()

    def _mostrar_clientes_en_treeview(self, clientes: list[dict[str, Any]]) -> None:
    ...
    def cargar_datos(self) -> None:
        ...
        self.combo_clientes["values"] = (
            ClientesServicio.obtener_nombres_clientes_para_busqueda()
        )
        self.combo_clientes.set("")

    def filtrar_clientes(self, event: Any | None = None) -> None:
        ...
        nombres_filtrados = ClientesServicio.obtener_nombres_clientes_para_busqueda(
            texto
        )
    ...
    def eliminar_cliente(self, event: Any | None = None) -> None:
        ...
        if messagebox.askyesno(
            "Confirmar eliminación", "¿Está seguro de eliminar este cliente?"
        ):
    ...
    def editar_celda(self, event: Any) -> None:
        ...
        frame_popup = tk.Frame(popup, bg=COLOR_FONDO_TABLA, padx=20, pady=20)
        ...
        frame_botones = tk.Frame(frame_popup, bg=COLOR_FONDO_TABLA)
        frame_botones.pack(pady=20)

        def guardar_cambios(event: Any | None = None) -> None:
            ...
            if not nuevo_texto:
                ...
                return
            exito, mensaje = ClientesServicio.actualizar_cliente(
                id_cliente, campo_db, nuevo_texto
            )
            if exito:
    ...
    def cargar_formulario(self, event: Any) -> None:
        seleccionado = self.tree.selection()
        if not seleccionado:
            return
