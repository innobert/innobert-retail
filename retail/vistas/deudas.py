...
def peso_colombiano(value: int) -> str:
...
class Deudas(tk.Frame):
    def __init__(self, padre: Any, controlador: Any) -> None:
        ...
        self.carrito_deuda: list[dict[str, Any]] = []  # Lista de productos para la deuda
        self.productos_cache: list[dict[str, Any]] = (
            []
        )  # Lista de diccionarios de productos (no se usa en paginación)
    ...
    def widgets(self) -> None:
        ...
        def cargar_img(nombre: str, size: tuple[int, int] = (30, 30)) -> ImageTk.PhotoImage | None:
            try:
                return ImageTk.PhotoImage(
                    Image.open(ruta_img / nombre).resize(size)
                )
            except Exception:
        ...
        self.btn_carrito = crear_boton(
            frame_detalles,
            "CARRITO",
            estilo=DEUDAS_BOTON_CARITO,
            image=self.img_carrito,
            compound="left",
            comando=lambda: self.controlador.abrir_carrito_deuda(self),
            padx=10,
            anchor="w",
            height=30,
            width=150,
        ...
        self.btn_facturas = crear_boton(
            ...
            comando=lambda: self.controlador.abrir_facturas_deudas(self),
            padx=10,
            anchor="w",
            height=30,
            width=150,
    ...
    def _renderizar_productos(self, productos: list[dict[str, Any]]) -> None:
        ...
        for producto in productos:
            ...
            # Imagen (resolver via rutas y fallback a default)
            try:
    ...
    def filtrar_productos_combobox(self, event: Any | None = None) -> None:
        ...
        nombres_filtrados = DeudasServicio.obtener_nombres_productos_para_busqueda(
            texto
        )
    ...
    def _solicitar_cantidad_producto(self, data: dict[str, Any], frame: Any) -> None:
        ...
        cantidad_en_carrito = sum(
            item["cantidad"]
            for item in self.carrito_deuda
        ...
        tk.Label(
            frame_cantidad, text="Cantidad:", font=("Helvetica", 12), bg=COLOR_FONDO
        ...
        def agregar() -> None:
            try:
                ...
                stock_inventario_actual = DeudasServicio.obtener_stock_actual(
                    id_producto
                )
                if stock_inventario_actual is None:
                    messagebox.showerror(
                        "Error", "No se pudo obtener el stock actual.", parent=top
                    )
                    return
                ...
                nuevo_carrito, mensaje, error = ServicioCarritoDeudas.agregar_al_carrito(
    ...
    # ----------------------------------------------------------------------
    def actualizar_combobox_clientes(self) -> None:
        nombres, _ = DeudasServicio.obtener_clientes_formateados()
        self.entry_cliente["values"] = nombres

    def _validar_y_limpiar_carrito_si_cambia_cliente(self, nuevo_id: int | None) -> bool:
        """Verifica si cambia de cliente y limpia el carrito si es necesario."""
        if not self.carrito_deuda:
            return True

        if (
            self.cliente_id_seleccionado is None
            or self.cliente_id_seleccionado != nuevo_id
        ):
            if messagebox.askyesno(
                ...
                self.actualizar_total_carrito_display()
                messagebox.showinfo(
                    "Carrito limpiado", "El carrito ha sido vaciado.", parent=self
                )
                return True
            else:
                return False
    ...
    def _al_entrar_cliente(self, event: Any | None = None) -> None:
            ...
            if self.carrito_deuda:
                if messagebox.askyesno(
                    ...
                    if self.cliente_id_seleccionado:
                        cliente_nombre = DeudasServicio.obtener_nombre_cliente_por_id(
        ...
        for c in clientes_filtrados:
            if c["nombre_completo"].lower() == texto.lower():
                if not self._validar_y_limpiar_carrito_si_cambia_cliente(
                    c["id_cliente"]
                ):
    ...
    def actualizar_total_carrito_display(self) -> None:
        ...
        try:
            ...
            logger.warning("Error al formatear total del carrito, usando $0")
            self.var_total_carrito.set("$0")

    def actualizar_canvas_productos(self) -> None:
        self.cargar_productos()

    def confirmar_deuda(self) -> None:
        if not self.carrito_deuda:
            messagebox.showwarning(
                "Carrito vacío", "No hay productos para crear la deuda.", parent=self
            )
            return
        ...
        if not self.cliente_id_seleccionado:
            messagebox.showerror(
                "Cliente requerido",
                "Debe seleccionar un cliente para crear la deuda.",
                parent=self,
            )
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Éxito",
                f"Deuda creada correctamente. ID: {result['id_deuda']}",
                parent=self,
            )
        except Exception as e:
            logger.exception("No se pudo crear la deuda")
            messagebox.showerror(
                "Error", f"No se pudo crear la deuda: {e}", parent=self
            )
