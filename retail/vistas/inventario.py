...
import logging
from retail.nucleo.base_datos import peso_colombiano
from retail.nucleo.servicios.inventario.servicio_inventario import (
    InventarioServicio,
)
...
class Inventario(tk.Frame):
    def __init__(self, padre: Any, controlador: Any) -> None:
        ...
        if not Path(self.img_folder).exists():
            Path(self.img_folder).mkdir(parents=True, exist_ok=True)

        self.ultimo_directorio = Path.home()
    ...
    def widgets(self) -> None:
        ...
        def cargar_img(nombre: str) -> ImageTk.PhotoImage | None:
            try:
        ...
        # Botón Agregar
        self.btn_agregar = crear_boton(
            lf_opciones,
            "  Agregar",
            estilo=BOTON_EXITO,
            image=img_add,
            compound="left",
            comando=self.agregar_producto,
            padx=10,
            anchor="w",
        )
        ...
        # Botón Historial
        self.btn_historial = crear_boton(
            lf_opciones,
            "  Historial",
            estilo=BOTON_INFO,
            image=img_historial,
            compound="left",
            comando=self.abrir_historial_inventario,
            padx=10,
            anchor="w",
        )
        ...
        # Botón Totales
        self.btn_totales = crear_boton(
            lf_opciones,
            "  Totales",
            estilo=BOTON_NEUTRO,
            image=img_total,
            compound="left",
            comando=self.abrir_totales_inventario,
            padx=10,
            anchor="w",
        )
        ...
        self.canvas = tk.Canvas(
            lf_productos, bg=COLOR_FONDO, highlightthickness=0, relief="flat"
        )
        self.canvas.grid(row=0, column=0, columnspan=2, sticky="nsew")

        # Scrollbar vertical
        self.scrollbar_v = ttk.Scrollbar(
            lf_productos, orient="vertical", command=self.canvas.yview
        )
        ...
        self.frame_paginacion = tk.Frame(lf_productos, bg=COLOR_FONDO)
    ...
    def mostrar_producto(
        ...
        frame_producto.grid(
            row=row, column=col, padx=separacion_x, pady=separacion_y, sticky="nsew"
        )
        ...
        try:
            imagen = Image.open(
                rutas(imagen_path) if not Path(imagen_path).is_absolute() else imagen_path
            )
            ...
            img_label.bind(
                "<Button-1>",
                lambda e, data=frame_producto.producto_data, f=frame_producto: self.mostrar_seleccion(
                    data, f
                ),
            )
            img_label.bind(
                "<Double-Button-1>",
                lambda e, data=frame_producto.producto_data, f=frame_producto: self.tl_editar(
                    data, f
                ),
            )
        except Exception as e:
            logger.exception("Error al cargar imagen de producto")
    ...
    def mostrar_seleccion(self, data: dict[str, Any], frame: Any | None = None) -> None:
        if (
    ...
    def _abrir_dialogo_producto(self, editar: bool = False, datos: dict[str, Any] | None = None) -> None:
        ...
        frame_principal = tk.Frame(top, bg=COLOR_FONDO)
        ...
        frame_campos = tk.Frame(frame_principal, bg=COLOR_FONDO)
        ...
        btn_guardar = crear_boton(
            ...
            "Guardar",
            estilo=BOTON_EXITO,
            comando=self._guardar_producto_desde_dialogo,
        )
        ...
        default_image_path = rutas(str(Path("fotos") / "default.png"))
        try:
            ...
            default_image = default_image.resize((300, 300), Image.Resampling.LANCZOS)
            ...
            logger.exception("Error al cargar imagen por defecto")

        btn_cargar_imagen = crear_boton(
            frame_principal,
            "Cargar Imágen",
            estilo=BOTON_IMPORTAR,
            comando=self.cargar_imagen,
        )
        btn_cargar_imagen.place(x=370, y=320)

        # Campos
        tk.Label(
            frame_campos, text="Producto", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
        ).pack(anchor="w")
        ...
        tk.Label(
            frame_campos, text="Precio", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
        ).pack(anchor="w")
        ...
        tk.Label(
            frame_campos, text="Costo", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
        ).pack(anchor="w")
        ...
        tk.Label(
            frame_campos, text="Stock", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
        ).pack(anchor="w")
        ...
        tk.Label(
            frame_campos, text="Estado", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
        ).pack(anchor="w")
    ...
    def _guardar_producto_desde_dialogo(self) -> None:
        try:
            ...
            if not all([producto, precio_str, costo_str, stock_str]):
                messagebox.showerror(
                    "Error", "Todos los campos son obligatorios", parent=self.top
                )
                return
            ...
            advertencia = InventarioServicio.verificar_rentabilidad(
                precio, costo, stock
            )
            if advertencia:
                if not messagebox.askyesno(
                    "Advertencia", advertencia["mensaje"], parent=self.top
                ):
                    return
            ...
            if self.top.editing:
                ...
                exito, mensaje = InventarioServicio.agregar_producto(
                    producto, precio, costo, stock, imagen
                )
    ...
    def tl_editar(self, data: dict[str, Any], frame: Any) -> None:
        self._abrir_dialogo_producto(editar=True, datos=data)

    def eliminar_producto_seleccionado(self) -> None:
        ...
        if messagebox.askyesno("Confirmar", f"¿Está seguro de eliminar '{producto}'?"):
            exito, mensaje = InventarioServicio.eliminar_producto(
    ...
    def cargar_imagen(self) -> None:
        try:
            ...
            dialog.destroy()
            if file_path:
                self.ultimo_directorio = Path(file_path).parent
                image = Image.open(file_path)
                image = image.resize((300, 300), Image.Resampling.LANCZOS)

                image_name = Path(file_path).name
                image_save_path = Path(self.img_folder) / image_name
                ...
                img_label = tk.Label(self.frame_img, image=self.image_tk, bg="white")
                img_label.pack(fill="both", expand=True)
                self.image_path = str(image_save_path)
        except Exception as e:
            messagebox.showerror(
                "Error", f"Error al cargar imagen: {e}", parent=self.top
            )
    ...
    # ----------------------------------------------------------------------
    def _actualizar_otras_vistas(self) -> None:
        parent = self.master
        while parent:
    ...
    def _activar_scroll_canvas(self, event: Any | None = None) -> None:
    ...
    def _desactivar_scroll_canvas(self, event: Any | None = None) -> None:
