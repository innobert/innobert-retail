...
def ver_deudas_pagadas(deudas_view: Any = None) -> None:
    ...
    def cargar_pagina() -> None:
        ...
        pagadas = ServicioPagadas.obtener_pagina(
    ...
    def generar_factura_deuda_pagada() -> None:
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning(
                "Advertencia",
        ...
        productos_detalle, cliente_nombre = ServicioPagadas.obtener_detalles_para_pdf(
            id_deuda
        )
        ...
        guardar_ultima_carpeta_pdf("pagadas", str(carpeta_usada))
    ...
    def abrir_historial_deuda_pagada() -> None:
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning(
                "Advertencia",
    ...
    btn_imprimir = crear_boton(
        ...
        fuente=FUENTE_BOTON_GRANDE,
        comando=generar_factura_deuda_pagada,
        padx=12,
        pady=6,
        anchor="w",
    )
    ...
    btn_historial = crear_boton(
        ...
        fuente=FUENTE_BOTON_GRANDE,
        comando=abrir_historial_deuda_pagada,
        padx=12,
        pady=6,
        anchor="w",
    )
    ...
    lf_total = tk.LabelFrame(
        ventana, text="Total Pagado", font=("Helvetica", 12, "bold"), bg=COLOR_FONDO
    )
    ...
    def actualizar_total_pagado() -> None:
