...
from retail.nucleo.configuraciones import COLOR_FONDO, COLOR_FONDO_TABLA
from retail.nucleo.servicios.deudas.servicio_historial_deudas import (
    ServicioHistorialDeudas,
)
...
def abrir_historial_deudas(
    parent: Any,
    nombre_cliente: str = "Cliente",
    cliente_rapido: Optional[str] = None,
    ...
    if id_deuda is not None:
        ...
        if not historial:
            messagebox.showinfo(
                "Información",
                f"No se encontró historial para la deuda #{id_deuda}.",
                parent=parent,
            )
            return
        cliente_nombre = (
            ...
            else ServicioHistorialDeudas.obtener_nombre_cliente_por_deuda(id_deuda)
        )
        numero_factura = ServicioHistorialDeudas.obtener_numero_factura_por_deuda(
            id_deuda
        )
        ...
        if nombre_cliente and nombre_cliente.isdigit():
            ...
            cliente_mostrar = (
                nombre_cliente
                if nombre_cliente
                else (cliente_rapido if cliente_rapido else "Cliente")
            )
        historial = ServicioHistorialDeudas.obtener_por_cliente(
            cliente_mostrar, id_cliente
        )
        if not historial:
            messagebox.showinfo(
                "Información",
                f"No se encontraron deudas para {cliente_mostrar}.",
                parent=parent,
            )
            return
    ...
    try:
        ...
        logger.warning("No se pudieron establecer atributos de ventana (transient/grab/topmost)")
    ...
    def _en_rueda_raton(event: Any) -> None:
        tree.yview_scroll(int(-1 * (event.delta / 120)), "units")

    tree.bind("<Enter>", lambda e: tree.bind_all("<MouseWheel>", _en_rueda_raton))
    ...
    tk.Frame(ventana, bg=COLOR_FONDO)
    ventana.mainloop()
