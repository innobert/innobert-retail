...
class ServicioHistorialVentas:
    ...
    def obtener_por_cliente(
        id_cliente: int, cliente_rapido: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def _procesar_filas(rows: List[Tuple[Any, ...]]) -> List[Dict[str, Any]]:
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            try:
                ...
                logger.debug("Error al calcular día de la semana: %s", fecha)
                dia_semana = ""
            # Manejo de valores nulos
            cantidad_str = str(cantidad) if cantidad is not None else ""
            subtotal_str = (
                f"${subtotal:,.0f}".replace(",", ".") if subtotal is not None else ""
            )
            monto_str = (
                f"${monto_recibido:,.0f}".replace(",", ".")
                if monto_recibido is not None
                else "0"
            )
            vuelto_str = (
                f"${vuelto:,.0f}".replace(",", ".") if vuelto is not None else "0"
            )
            historial.append(
                {
                    "idx": idx,
