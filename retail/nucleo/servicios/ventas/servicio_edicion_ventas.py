...
class ServicioEdicionVentas:
    ...
    @staticmethod
    def obtener_detalles_factura(id_ventas: int, conn=None) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def obtener_info_factura(id_ventas: int, conn=None) -> Tuple[float, float, float]:
        """
    ...
    @staticmethod
    def eliminar_detalle_venta(id_detalle: int, usuario: str, monto_recibido: float, conn=None, cursor=None) -> None:
        """
        ...
        try:
            ...
            trashed_timestamp = ServicioEdicionVentas._mover_a_papelera_si_vacia(id_ventas, usuario,
    ...
    @staticmethod
    def _mover_a_papelera_si_vacia(id_ventas: int, usuario: str, conn=None, cursor=None):
        """
            ...
            if count == 0:
                ...
                cursor.execute(
                    ...
                    (id_ventas, numero_factura, cliente_id, cliente_rapido, fecha, hora, total,
                     ganancia, monto_recibido, vuelto, usuario, timestamp_str, motivo)
                )
    ...
    @staticmethod
    def editar_cantidad_detalle(id_detalle: int, nueva_cantidad: int, usuario: str, monto_recibido: float,
        ...
        try:
            ...
            if not detalle:
                raise ValueError("Detalle no encontrado")
            id_ventas, id_producto, cant_antigua, sub_antiguo, stock_actual, precio_unit = detalle
    ...
    @staticmethod
    def agregar_producto_a_venta(id_ventas: int, id_producto: int, cantidad: int, usuario: str,
                                  monto_recibido: float, conn=None, cursor=None) -> None:
        """
        ...
        try:
            cursor.execute("SELECT precio, stock, producto FROM inventario WHERE id_producto = ?", (id_producto,))
            ...
            # Actualizar stock
            cursor.execute("UPDATE inventario SET stock = stock - ? WHERE id_producto = ?", (cantidad, id_producto))
    ...
    # ----------------------------------------------------------------------
    @staticmethod
    def obtener_productos_paginado(filtro: str = "", offset: int = 0, limit: int = 12, conn=None) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def contar_productos_con_filtro(filtro: str = "", conn=None) -> int:
