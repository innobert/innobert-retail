...
class ServicioPagadas:
    ...
    @staticmethod
    def contar_pagadas(filtro_cliente: str = "") -> Any:
    ...
    def obtener_pagina(
        ...
        for row in rows:
            ...
            saldo_pagado = (
                float(total) - float(saldo) if saldo is not None else float(total)
            )
            pagadas.append(
                {
                    "id_deuda": id_deuda,
    ...
    @staticmethod
    def calcular_total_pagado(filtro_cliente: str = "") -> Any:
        ...
        with conexion() as conn:
            ...
            if filtro_cliente:
                ...
                cursor.execute(
                    "SELECT SUM(total - saldo) FROM deudas WHERE estado = 'PAGADA'"
                )
    ...
    @staticmethod
    def obtener_detalles_para_pdf(id_deuda: int) -> Tuple[List[tuple[Any, ...]], str]:
