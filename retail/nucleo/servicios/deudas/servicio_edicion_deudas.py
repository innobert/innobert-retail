...
class ServicioEdicionDeudas:
    ...
    @staticmethod
    def obtener_detalles_deuda(id_deuda: int, conn: Any | None = None) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def obtener_info_deuda(id_deuda: int, conn: Any | None = None) -> Tuple[float, float]:
        ...
        try:
            ...
            with ctx as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT total, saldo FROM deudas WHERE id_deuda = ?", (id_deuda,)
                )
                row = cursor.fetchone()
    ...
    @staticmethod
    def eliminar_producto_deuda(
        id_detalle: int, usuario: str, conn: Any | None = None, cursor: Any | None = None
    ) -> None:
        """
        ...
        with ctx as conn:
            ...
            # Eliminar el detalle
            cursor.execute(
                "DELETE FROM detalle_deuda WHERE id_detalle = ?", (id_detalle,)
            )

            # Recalcular total y saldo de la deuda
            cursor.execute(
                "SELECT SUM(subtotal) FROM detalle_deuda WHERE id_deuda = ?",
                (id_deuda,),
            )
            nuevo_total = cursor.fetchone()[0] or 0.0
            cursor.execute(
                "SELECT COALESCE(SUM(monto), 0) FROM pagos_deuda WHERE id_deuda = ?",
                (id_deuda,),
            )
    ...
    def editar_cantidad_detalle(
        id_detalle: int, nueva_cantidad: int, usuario: str, conn: Any | None = None, cursor: Any | None = None
    ) -> None:
        """
    ...
    def obtener_productos_paginado(
        filtro: str = "", offset: int = 0, limit: int = 12
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def contar_productos_con_filtro(filtro: str = "") -> Any:
