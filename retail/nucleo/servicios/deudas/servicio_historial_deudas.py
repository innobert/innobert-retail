...
class ServicioHistorialDeudas:
    ...
    def obtener_numero_factura_por_deuda(id_deuda: int) -> str:
        ...
        with conexion() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT numero_factura FROM deudas WHERE id_deuda = ?", (id_deuda,)
            )
            row = cursor.fetchone()
    ...
    def obtener_por_cliente(
        nombre_cliente: str, id_cliente: int | None = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def _procesar_filas(rows: List[Any], id_deuda_fijo: int | None = None) -> List[Dict[str, Any]]:
        ...
        saldo_acumulado = 0.0
        historial = []
        producto_subtotales: dict[str, float] = {}
        total_abonos = 0
        for idx, row in enumerate(rows, start=1):
            ...
            try:
                ...
                logger.debug("Error al calcular día de la semana para fecha: %s", fecha)
                dia_semana = ""
            ...
            if accion_normalizado == "ABONO":
                ...
                if accion_normalizado in ("DEUDA", "AGREGADO"):
                    producto_subtotales[id_producto] = producto_subtotales.get(
                        id_producto, 0
                    ) + (subtotal or 0)
                else:
            ...
            cantidad_total = (
                cantidad
                if accion_normalizado not in ("ABONO", "ELIMINADO") and cantidad
                else 0
            )
            ...
            producto_mostrar = (
                ...
                else ("ABONO" if accion_normalizado == "ABONO" else "")
            )
            subtotal_text = ""
            if (
                accion_normalizado not in ("ABONO", "ELIMINADO")
            ...
            historial.append(
                {
                    ...
                    "recibido": (
                        f"${recibido:,.0f}".replace(",", ".") if recibido else ""
                    ),
