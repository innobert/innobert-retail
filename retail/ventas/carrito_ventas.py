...
from retail.nucleo.configuraciones import (COLOR_FONDO, COLOR_FONDO_EDITAR, COLOR_FONDO_TABLA, COLOR_AZUL, COLOR_VERDE, COLOR_ROJO, FUENTE_ETIQUETA, crear_boton, VENTAS_BOTON_PELIGRO, VENTAS_BOTON_EXITO, FUENTE_BOTON_NEGRITA)
...
def ver_carrito(ventas_view: Any) -> None:
    ...
    frame_main = tk.Frame(top, bg=COLOR_FONDO_EDITAR)
    ...
    for cliente, items_con_indice in clientes.items():
        ...
        lbl_total_valor = tk.Label(
            panel_total,
            text=peso_colombiano(total_cliente),
        ...
        def eliminar_producto(tree: ttk.Treeview = tree, iid_map: dict[str, Any] = iid_map, cliente: str = cliente) -> None:
            seleccion = tree.selection()
            if not seleccion:
                messagebox.showwarning(
                    "Advertencia", "Seleccione un producto.", parent=top
                )
                return
            iid = seleccion[0]
            ...
            if not any(
                ...
                or (not item["cliente"] and cliente == "Venta rápida")
                for item in carrito
            ):
                top.destroy()
                messagebox.showinfo(
        ...
        def actualizar_vuelto_cliente() -> None:
            try:
                monto = float(monto_var.get().strip()) if monto_var.get().strip() else 0
            except ValueError:
                monto = 0
            total_actual = float(
                lbl_total_valor.cget("text").replace("$", "").replace(".", "")
            )
            ...
            try:
                ...
                logger.warning("No se pudo almacenar monto_recibido en la vista")
        ...
        def editar_cantidad(event: Any, tree: ttk.Treeview = tree, iid_map: dict[str, Any] = iid_map, cliente: str = cliente) -> None:
            seleccion = tree.selection()
            if not seleccion:
                return
            iid = seleccion[0]
            ...
            if stock_actual is None:
                messagebox.showerror(
                    "Error", "No se pudo obtener el stock.", parent=top
                )
                return
            ...
            tk.Label(
                frame_cant, text="Cantidad:", font=("Helvetica", 12), bg=COLOR_FONDO_EDITAR
            ...
            def guardar() -> None:
                try:
                    ...
                    if stock_act2 is None:
                        ...
                        return
                    valido2, _, msg2 = (
                        ServicioCarritoVentas.validar_cantidad_para_edicion(
                    ...
                    if not valido2:
                        ...
                        return
                    ServicioCarritoVentas.actualizar_cantidad_en_carrito(
                        carrito, orig_idx, nueva
                    )
                    tree.set(iid, "cantidad", nueva)
                    tree.set(iid, "subtotal", peso_colombiano(prod["subtotal"]))
                    nuevo_total_cliente = sum(
                        item[1]["subtotal"] for item in clientes.get(cliente, [])
                    )
                    totales_por_cliente[cliente]["lbl_total"].config(
                        text=peso_colombiano(nuevo_total_cliente)
                    )
    ...
    frame_confirmar = tk.Frame(frame_main, bg=COLOR_FONDO_EDITAR)
    frame_confirmar.pack(fill="x", pady=(10, 0))

    def confirmar_venta() -> None:
        if not ventas_view.carrito:
            messagebox.showwarning(
                "Carrito vacío", "No hay productos en el carrito.", parent=top
            )
            return
        ...
        total_carrito = ServicioCarritoVentas.calcular_total_general(
            ventas_view.carrito
        )
        ...
        id_cliente = None
        if ventas_view.entry_cliente.get().strip() and hasattr(
            ventas_view, "cliente_id_seleccionado"
        ):
        ...
        try:
            ...
            ventas_view.after(
                100, lambda: ventas_view.actualizar_total_carrito_display()
            )
            messagebox.showinfo(
                "✓ Venta Confirmada",
            ...
            logger.exception("No se pudo registrar la venta")
            btn_confirmar.config(state="normal", cursor="hand2")
            top.config(cursor="")
            messagebox.showerror(
                "Error", f"No se pudo registrar la venta: {e}", parent=top
            )
