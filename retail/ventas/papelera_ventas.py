...
Ventana para visualizar las ventas eliminadas (papelera de reciclaje).
...
from retail.nucleo.configuraciones import (COLOR_FONDO, COLOR_FONDO_TABLA, COLOR_AZUL, COLOR_ROJO, crear_boton, VENTAS_BOTON_NEUTRO, VENTAS_BOTON_CERRAR, VENTAS_BOTON_NAV)
from retail.nucleo.servicios.ventas.servicio_papelera_ventas import (
    ServicioPapeleraVentas,
)

logger = logging.getLogger(__name__)


def peso_colombiano(value: Any) -> str:
    try:
...
def ver_papelera_ventas(parent: Any) -> None:
    ventana = tk.Toplevel(parent)
    ventana.title("Papelera de Ventas")
    ventana.geometry("1100x650+50+20")
    ventana.configure(bg=COLOR_FONDO)
    ventana.resizable(False, False)
    ventana.maxsize(1100, 650)
    ...
    if eliminados > 0:
        logger.info("Papelera Ventas: %d registros antiguos eliminados automáticamente.", eliminados)
    ...
    def actualizar_totales() -> None:
        ...
        total_registros = ServicioPapeleraVentas.contar_papelera(filtro_actual)
        total_paginas = max(
    ...
    def cargar_pagina() -> None:
        ...
        registros = ServicioPapeleraVentas.obtener_pagina(
