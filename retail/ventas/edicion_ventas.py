...
# ---------------------------------------------------------------------------
def calcular_cantidad_maxima(
    total_actual: float, precio_unitario: float, monto_recibido: float
) -> int:
...
# ---------------------------------------------------------------------------
def _solicitar_monto_recibido(
    ventana_padre: Any, cliente: str, total_actual: float, monto_anterior: float
) -> Optional[float]:
    ...
    monto_resultado = [
        None
    ]  # Usamos lista mutable para modificar desde funciones anidadas
    var_vuelto = tk.StringVar(
        value=f"${monto_anterior - total_actual:,.0f}".replace(",", ".")
    )
    ...
    tk.Label(
        frame_main,
        text=f"Total actual: ${total_actual:,.0f}".replace(",", "."),
    ...
    tk.Label(
        frame_main,
        text=f"Monto anterior: ${monto_anterior:,.0f}".replace(",", "."),
        font=("Helvetica", 11),
        bg=COLOR_FONDO,
        fg="#666666",
    ).pack(pady=(0, 20))
    ...
    frame_entrada = tk.Frame(frame_main, bg=COLOR_FONDO)
    ...
    frame_vuelto = tk.Frame(frame_main, bg=COLOR_FONDO)
    frame_vuelto.pack(pady=(0, 20))
    tk.Label(
        frame_vuelto, text="Vuelto:", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
    ).pack(side="left", padx=(0, 10))
    tk.Label(
        frame_vuelto,
        textvariable=var_vuelto,
        font=("Helvetica", 14, "bold"),
        bg=COLOR_FONDO,
        fg="#0B6623",
        width=15,
    ...
    def actualizar_vuelto(*args: Any) -> None:
        try:
        ...
        try:
            ...
            if monto <= 0:
                messagebox.showwarning(
                    "Monto inválido", "Debe ser mayor a 0.", parent=ventana
                )
                return
            if monto < total_actual:
                messagebox.showerror(
                    "Monto insuficiente",
                    f"El monto ${monto:,.0f} es menor al total ${total_actual:,.0f}.",
                    parent=ventana,
                )
                return
            ...
            messagebox.showwarning(
                "Valor inválido", "Ingrese un número.", parent=ventana
            )
...
# ---------------------------------------------------------------------------
def _mostrar_selector(
    ventana_padre: Any, id_ventas: int, cliente: str, monto_recibido: float, usuario_actual: str, callbacks: Optional[dict[str, Any]] = None
) -> None:
    selector = tk.Toplevel(ventana_padre)
    selector.title("Opciones de factura")
    selector.geometry("420x220+500+200")
    selector.configure(bg=COLOR_FONDO)
    ...
    tk.Label(
        selector,
        text="¿Qué deseas hacer con esta factura?",
        font=("Helvetica", 14, "bold"),
        bg=COLOR_FONDO,
        pady=14,
    ).pack(fill="x")

    frame_botones = tk.Frame(selector, bg=COLOR_FONDO)
    frame_botones.pack(fill="x", padx=20, pady=10)

    def abrir_editar() -> None:
        selector.destroy()
        _abrir_ventana_editar_factura(
            ventana_padre, id_ventas, cliente, monto_recibido, usuario_actual, callbacks
        )
...
# ---------------------------------------------------------------------------
def _abrir_ventana_editar_factura(
    ventana_padre: Any, id_ventas: int, cliente: str, monto_recibido: float, usuario_actual: str, callbacks: Optional[dict[str, Any]] = None
) -> None:
    ...
    frame_info = tk.Frame(top, bg=COLOR_FONDO_EDITAR, pady=8)
    ...
    tk.Label(
        frame_info, text="Monto recibido:", font=("Helvetica", 14, "bold"), bg=COLOR_FONDO_EDITAR
    ).pack(side="left", padx=(0, 8))
    ...
    tk.Label(
        frame_info, text="Vuelto:", font=("Helvetica", 14, "bold"), bg=COLOR_FONDO_EDITAR
    ).pack(side="left", padx=(0, 8))
    ...
    frame_buscar = tk.Frame(top, bg=COLOR_FONDO_EDITAR)
    ...
    style.configure(
        "EditarVenta.Treeview.Heading",
        font=FUENTE_ETIQUETA,
        background=COLOR_AZUL,
        foreground="#ffffff",
    )
    style.configure(
        "EditarVenta.Treeview",
        font=("Helvetica", 11),
        rowheight=40,
        background="#F8FAFB",
    )
    ...
    tree = ttk.Treeview(
        frame_tabla,
        columns=("id_detalle", "producto", "cantidad", "precio", "subtotal"),
        show="headings",
        style="EditarVenta.Treeview",
    )
    ...
    # Botones de acción
    frame_acciones = tk.Frame(top, bg=COLOR_FONDO_EDITAR)
    ...
    def cargar_datos() -> None:
        try:
            detalles = ServicioEdicionVentas.obtener_detalles_factura(
                id_ventas, conn=conn
            )
            total_actual, _, _ = ServicioEdicionVentas.obtener_info_factura(
                id_ventas, conn=conn
            )
    ...
    def eliminar_producto() -> None:
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar '{producto}'?", parent=top):
            try:
                ...
                messagebox.showerror(
                    "Error", f"No se pudo eliminar el producto:\n{err}", parent=top
                )
    ...
    def editar_cantidad(event: Any) -> None:
        ...
        info_frame = tk.Frame(popup, bg=COLOR_FONDO_EDITAR)
        ...
        tk.Label(
            info_frame,
            text=f"Cantidad permitida: {cantidad_permitida}",
            font=("Helvetica", 12),
            bg=COLOR_FONDO_EDITAR,
            fg="#0D47A1",
        ).pack(anchor="w", padx=10)
        if cantidad_permitida == 0:
            tk.Label(
                info_frame,
                text="No puede aumentar la cantidad adicional con el monto recibido actual.",
        ...
        lbl_mensaje = tk.Label(
            popup, text="", font=("Helvetica", 10), bg=COLOR_FONDO_EDITAR, fg="#C62828"
        )
        ...
        def guardar() -> None:
            ...
            if not texto.isdigit():
                messagebox.showwarning(
                    "Cantidad inválida",
                    "Ingrese un número entero válido mayor a 0.",
                    parent=popup,
                )
                return
            nueva = int(texto)
            if nueva <= 0:
                messagebox.showwarning(
                    "Cantidad inválida", "Ingrese un número mayor a 0.", parent=popup
                )
                return
            if nueva == cantidad_actual:
                messagebox.showinfo(
                    "Sin cambios",
                    "No se realizó ningún cambio porque la cantidad es igual a la actual.",
                    parent=popup,
                )
                popup.destroy()
                return
            ...
            try:
                ...
                if nueva == limite_final and cantidad_permitida > 0:
                    messagebox.showinfo(
                        "Límite utilizado",
                        "Ha usado la cantidad permitida completa. No quedan unidades adicionales disponibles.",
                        parent=popup,
                    )
                else:
                    messagebox.showinfo(
                        "Éxito", "Cantidad actualizada correctamente.", parent=popup
                    )
                cargar_datos()
                popup.destroy()
            except Exception as err:
                messagebox.showerror(
                    "Error", f"No se pudo actualizar la cantidad:\n{err}", parent=popup
                )

        def actualizar_mensaje(*args: Any) -> None:
            texto = entry_cant.get().strip()
            if not texto:
            ...
            if not texto.isdigit():
                lbl_mensaje.config(
                    text=f"⚠️ Solo se permiten números enteros entre 1 y {limite_final}"
                )
                return
            nueva = int(texto)
            if nueva > limite_final:
...
def _abrir_ventana_agregar_productos(
    ...
    frame_canvas = tk.LabelFrame(
        top, text="Productos disponibles", font=FUENTE_ETIQUETA, bg=COLOR_FONDO
    )
    ...
    def cargar_productos_pagina() -> None:
        ...
        productos = ServicioEdicionVentas.obtener_productos_paginado(
        ...
        for idx, prod in enumerate(productos):
            ...
            try:
                ...
                logger.exception("Error cargando imagen para %s", prod.get("producto"))
                lbl_img = tk.Label(
                    frame_prod, text="Sin imagen", bg="white", font=("Helvetica", 9)
                )
...
def _agregar_producto_a_venta(
    producto: dict[str, Any], parent: Any, id_ventas: int, monto_recibido: float, usuario: str, callbacks: Optional[dict[str, Any]], recargar_callback: Callable[[], None]
) -> None:
    total_actual, _, _ = ServicioEdicionVentas.obtener_info_factura(id_ventas)
    precio_unit = producto["precio"]
    cantidad_maxima = calcular_cantidad_maxima(
        total_actual, precio_unit, monto_recibido
    )
    ...
    tk.Label(
        info_frame,
        text=f"Cantidad permitida: {cantidad_maxima}",
        font=FUENTE_ETIQUETA,
        bg=COLOR_FONDO_EDITAR,
        fg="#0D47A1",
        anchor="w",
    ...
    if cantidad_maxima > 0:
        tk.Label(
            info_frame,
            text="Si usa toda la cantidad permitida, quedarán 0 unidades adicionales disponibles.",
        ...
        tk.Label(
            info_frame,
            text="⚠️ No puede agregar más unidades con el monto recibido actual.",
            font=("Helvetica", 11, "italic"),
            bg=COLOR_FONDO_EDITAR,
            fg="#C62828",
            wraplength=380,
    ...
    def confirmar() -> None:
        try:
            ...
            if cantidad <= 0:
                lbl_mensaje.config(
                    text="Cantidad inválida: ingrese un número mayor a cero."
                )
                return
            if cantidad > producto["stock"]:
                lbl_mensaje.config(
                    text=f"Stock insuficiente: solo hay {producto['stock']} unidades disponibles."
                )
                return
            ...
            ServicioEdicionVentas.agregar_producto_a_venta(
                id_ventas, producto["id_producto"], cantidad, usuario, monto_recibido
            )
    ...
    def actualizar_mensaje(*args: Any) -> None:
        try:
            ...
            if cant <= 0:
                ...
                lbl_mensaje.config(
                    text=f"⚠️ El máximo permitido con el monto actual es {cantidad_maxima}."
                )
            else:
                lbl_mensaje.config(text="")
        except ValueError:
...
# ---------------------------------------------------------------------------
def _notificar_cambios(callbacks: Optional[dict[str, Any]]) -> None:
...
# ---------------------------------------------------------------------------
def abrir_ventana_edicion_factura(
    ventana_padre: Any, id_ventas: int, cliente: str, usuario_actual: str, callbacks: Optional[dict[str, Any]] = None
) -> None:
    """
    Punto de entrada para la edición de una factura de venta.
    """
    total_actual, monto_anterior, _ = ServicioEdicionVentas.obtener_info_factura(
        id_ventas
    )

    monto_recibido = _solicitar_monto_recibido(
        ventana_padre, cliente, total_actual, monto_anterior
    )
