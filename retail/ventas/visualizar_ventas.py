...
from retail.nucleo.configuraciones import (COLOR_FONDO, COLOR_AZUL, crear_boton, VENTAS_BOTON_CERRAR, FUENTE_BOTON_NEGRITA)
from retail.nucleo.servicios.ventas.servicio_visualizar_ventas import (
    ServicioVisualizarVentas,
)
...
def ver_detalle_venta(parent: Any, id_ventas: int) -> None:
    """
    ...
    try:
        ...
        logger.warning("No se pudo parsear la fecha %s", datos.get("fecha"))
        dia_semana = ""
    ...
    main_frame = tk.Frame(top, bg=COLOR_FONDO, padx=15, pady=15)
    ...
    tk.Label(
        ...
        bg="#FFFFFF",
        fg="#333",
    ).grid(row=0, column=2, sticky="w", padx=(30, 5), pady=4)
    tk.Label(
        info_grid,
        text=datos["cliente"],
        font=("Helvetica", 12),
    ...
    tk.Label(
        info_grid, text=datos["fecha"], font=("Helvetica", 12), bg="#FFFFFF", fg="#333"
    ).grid(row=1, column=1, sticky="w", padx=5, pady=4)

    tk.Label(
        info_grid, text="Hora:", font=("Helvetica", 12, "bold"), bg="#FFFFFF", fg="#333"
    ).grid(row=1, column=2, sticky="w", padx=(30, 5), pady=4)
    tk.Label(
        info_grid, text=datos["hora"], font=("Helvetica", 12), bg="#FFFFFF", fg="#333"
    ).grid(row=1, column=3, sticky="w", padx=5, pady=4)

    tk.Label(
        info_grid, text="Día:", font=("Helvetica", 12, "bold"), bg="#FFFFFF", fg="#333"
    ).grid(row=2, column=0, sticky="w", padx=5, pady=4)
    tk.Label(
        info_grid, text=dia_semana, font=("Helvetica", 12), bg="#FFFFFF", fg="#333"
    ...
    tk.Label(
        info_grid,
        text=peso_colombiano(datos["total"]),
        font=("Helvetica", 12, "bold"),
        bg="#FFFFFF",
        fg="#0B6623",
    ...
    tk.Label(
        ...
        bg="#FFFFFF",
        fg="#333",
    ).grid(row=3, column=2, sticky="w", padx=(30, 5), pady=4)
    tk.Label(
        info_grid,
        text=peso_colombiano(datos["monto_recibido"]),
        font=("Helvetica", 12, "bold"),
        bg="#FFFFFF",
        fg="#0D47A1",
    ...
    tk.Label(
        info_grid,
        text=peso_colombiano(datos["vuelto"]),
        font=("Helvetica", 12, "bold"),
        bg="#FFFFFF",
        fg="#0B6623",
    ...
    tree = ttk.Treeview(
        frame_tabla, columns=columns, show="headings", style="DetalleVenta.Treeview"
    )
