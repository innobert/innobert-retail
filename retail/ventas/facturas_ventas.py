...
def ver_facturas(parent: tk.Tk) -> None:
    ...
    columnas = ("ID", "N°", "N° Factura", "Cliente", "Productos", "Recibido", "Vuelto", "Total", "Día", "Hora", "Fecha", "Zona")
    ...
    def actualizar_totales():
        ...
        total_paginas = max(1, (total_facturas + facturas_por_pagina - 1) // facturas_por_pagina)
    ...
    def cargar_pagina():
        ...
        facturas = ServicioFacturasVentas.obtener_pagina_facturas(offset, facturas_por_pagina, filtro_actual)
    ...
    # Cargar imágenes (usando resource_path para que funcione al empaquetar)
    def cargar_img(nombre):
        try:
            img_path = resource_path(os.path.join("img", nombre))
    ...
    def generar_factura():
        ...
        if not seleccionado:
            messagebox.showwarning("Advertencia", "Seleccione una factura para generar.", parent=ventana)
            return
        ...
        if not productos_detalle:
            messagebox.showerror("Error", f"No se encontraron productos para la factura {id_ventas}", parent=ventana)
            return

        numero_factura = numero_factura_mostrado if numero_factura_mostrado else f"FACT-{numero_secuencial}"
        ...
        guardar_ultima_carpeta_pdf('ventas', os.path.dirname(archivo_pdf))
        ...
        for prod, cant, sub, hora_prod, fecha_prod, _ in productos_detalle:
            ...
            nueva_y, lineas_usadas = draw_wrapped_text(
                c, prod_text, 40, y_fila, max_width_producto, "Helvetica", 11, line_height=14
            )
    ...
    def abrir_historial_cliente():
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning("Advertencia", "Seleccione una factura para ver el historial.", parent=ventana)
            return
        item = seleccionado[0]
        valores = tree.item(item, "values")
        id_ventas = valores[0]
        cliente_nombre = valores[3]
        parent.controlador.abrir_historial_venta(ventana, id_ventas=id_ventas, nombre_cliente=cliente_nombre, facturas_window=ventana)

    # --- Eliminar factura ---
    def eliminar_factura():
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning("Advertencia", "Seleccione una factura para eliminar.", parent=ventana)
            return
        item = seleccionado[0]
        valores = tree.item(item, "values")
        id_ventas = int(valores[0])
        if messagebox.askyesno("Confirmar", "¿Está seguro de eliminar esta factura?\n\nSe moverá a la Papelera de Ventas.", parent=ventana):
            success = ServicioFacturasVentas.eliminar_factura(id_ventas, usuario_elimino=parent.controlador.usuario_actual)
            if success:
                actualizar_totales()
                cargar_pagina()
                messagebox.showinfo("Éxito", "Factura eliminada y movida a Papelera.", parent=ventana)
            else:
                messagebox.showerror("Error", "No se pudo eliminar la factura seleccionada.", parent=ventana)
    ...
    def editar_factura(event=None):
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning("Advertencia", "Seleccione una factura para editar.", parent=ventana)
            return
        item = seleccionado[0]
        valores = tree.item(item, "values")
        id_ventas = int(valores[0])
        ...
        parent.controlador.abrir_edicion_factura(ventana, id_ventas, cliente, parent.controlador.usuario_actual, callbacks)

    # --- VER DETALLE (nuevo) ---
    def _ver_detalle_factura():
        seleccionado = tree.selection()
        if not seleccionado:
            messagebox.showwarning("Advertencia", "Seleccione una factura para ver los detalles.", parent=ventana)
            return
        item = seleccionado[0]
        valores = tree.item(item, "values")
        id_ventas = int(valores[0])
        try:
            ...
            messagebox.showerror("Error", f"No se pudo cargar el módulo de visualización:\n{e}", parent=ventana)

    # --- Botones de opciones (VER ahora es el primero) ---
    btn_visualizar = tk.Button(lf_opciones, text="VER", image=img_visualizar, compound="left",
                               font=("Helvetica", 14, "bold"), bg="#FF9800", fg="white",
                               relief="ridge", bd=3, cursor="hand2", padx=12, pady=6, anchor="w",
                               command=_ver_detalle_factura)
    btn_visualizar.image = img_visualizar
    btn_visualizar.pack(pady=7, padx=10, fill="x")

    btn_imprimir = tk.Button(lf_opciones, text="IMPRIMIR", image=img_imprimir, compound="left",
    ...
    btn_historial = tk.Button(lf_opciones, text="HISTORIAL", image=img_historial, compound="left",
                              font=("Helvetica", 14, "bold"), bg="#6A1B9A", fg="white",
                              relief="ridge", bd=3, cursor="hand2", padx=12, pady=6, anchor="w",
                              command=abrir_historial_cliente)
    if img_historial:
        btn_historial.image = img_historial
    btn_historial.pack(pady=7, padx=10, fill="x")

    btn_eliminar = tk.Button(lf_opciones, text="ELIMINAR", image=img_eliminar, compound="left",
                             font=("Helvetica", 14, "bold"), bg="#C62828", fg="white",
                             relief="ridge", bd=3, cursor="hand2", padx=12, pady=6, anchor="w",
                             command=eliminar_factura)
    btn_eliminar.image = img_eliminar
    btn_eliminar.pack(pady=7, padx=10, fill="x")

    btn_papelera = tk.Button(lf_opciones, text="PAPELERA", image=img_papelera, compound="left",
                             font=("Helvetica", 14, "bold"), bg="#607D8B", fg="white",
                             relief="ridge", bd=3, cursor="hand2", padx=12, pady=6, anchor="w",
                             command=abrir_papelera)
    if img_papelera:
        btn_papelera.image = img_papelera
    btn_papelera.pack(pady=7, padx=10, fill="x")

    btn_logo = tk.Button(lf_opciones, text="LOGO", image=img_cambiar, compound="left",
                         font=("Helvetica", 14, "bold"), bg="#1976D2", fg="white",
                         relief="ridge", bd=3, cursor="hand2", padx=12, pady=6, anchor="w",
                         command=lambda: cambiar_logo(ventana))
    if img_cambiar:
        btn_logo.image = img_cambiar
    btn_logo.pack(pady=7, padx=10, fill="x")

    # --- Total de ventas (similar al total de deudas) ---
    lf_total_ventas = tk.LabelFrame(ventana, text="Total Ventas", font=("Helvetica",12,"bold"), bg="#E6D9E3")
    lf_total_ventas.place(x=1070, y=540, width=200, height=90)
    var_total_ventas = tk.StringVar(value="$0")
    lbl_total_ventas = tk.Label(lf_total_ventas, textvariable=var_total_ventas, font=("Helvetica",16,"bold"),
                                bg="#E6D9E3", fg="#008B8B")
    lbl_total_ventas.pack(expand=True, fill="both", pady=10)

    def actualizar_total_ventas():
