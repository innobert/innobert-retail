...
from retail.nucleo.configuraciones import (COLOR_FONDO, COLOR_FONDO_TABLA, TAMANO_VENTANA)
from retail.nucleo.servicios.ventas.servicio_historial_ventas import (
    ServicioHistorialVentas,
)
...
def abrir_historial_ventas(
    ...
    historial, titulo = _obtener_historial_y_titulo(
        parent, id_ventas, nombre_cliente, facturas_window
    )
    if not historial:
        return
    ...
    _configurar_informacion_emergente(tree)
...
def _obtener_historial_y_titulo(
    ...
    if id_ventas is not None:
        ...
        historial = ServicioHistorialVentas.obtener_por_cliente(
...
def _crear_ventana(parent: tk.Tk, titulo: str) -> tk.Toplevel:
    ...
    ventana.geometry(TAMANO_VENTANA + "+10+10")
...
def _crear_tabla(ventana: tk.Toplevel) -> tuple[ttk.Treeview, tk.Frame]:
    """
...
# ----------------------------------------------------------------------------
def _configurar_informacion_emergente(tree: ttk.Treeview) -> None:
    """
    ...
    def mostrar_tooltip(event: Any) -> None:
    ...
    def ocultar_tooltip(event: Any) -> None:
        tooltip.place_forget()
