...
class Mes(tk.Frame):
    ...
    def widgets(self) -> None:
        ...
        self.tree.column("num_mes", width=50, anchor="center")
        self.tree.column("mes", width=150, anchor="center")
    ...
    def cargar_datos(self) -> None:
        ...
        try:
            if self.periodos_data is None:
                self.periodos_data = ServicioMensual.obtener_periodos()
            self.total_periodos = len(self.periodos_data)
            self.total_paginas = max(
                1,
                (self.total_periodos + self.periodos_por_pagina - 1)
                // self.periodos_por_pagina,
            )
            if self.pagina_actual > self.total_paginas:
            ...
            total_ganancia, total_ventas = ServicioMensual.obtener_totales_globales()
            self.after(0, self._actualizar_totales, total_ganancia, total_ventas)

        except Exception as e:
            logger.exception("Error cargando períodos")
            self.after(
                0,
    ...
    def _actualizar_tabla(self, periodos: list[Any]) -> None:
    ...
    def generar_pdf_mes(self) -> None:
        ...
        if archivo_pdf:
            ...
            try:
                ...
                logger.warning("No se pudo insertar el logo en la factura mensual")
            ...
            y = height - 120
            c.drawString(40, y, f"Período N°: {num_mes}")
            c.drawString(40, y - 25, f"Nombre: {nombre_meses}")
